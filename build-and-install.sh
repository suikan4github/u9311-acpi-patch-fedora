...
function make_and_install_patch() {
    ...
    if [[ -e /run/ostree-booted ]] && command -v rpm-ostree >/dev/null 2>&1; then
        ...
        INITRAMFS_ENABLE_OUTPUT=$(sudo rpm-ostree initramfs --enable 2>&1)
        INITRAMFS_ENABLE_RC=$?
        printf '%s\n' "${INITRAMFS_ENABLE_OUTPUT}"
        if [ ${INITRAMFS_ENABLE_RC} -ne 0 ] \
            && ! grep -q "already enabled" <<< "${INITRAMFS_ENABLE_OUTPUT}"; then
            return 1
        fi
